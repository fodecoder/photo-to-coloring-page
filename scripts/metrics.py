"""Reference-comparison and raster-shape diagnostics for dev-time engine tuning.

This is a development diagnostic tool, not part of the project's shipped
acceptance criteria -- see ``coloring_page.validate`` for that (the
project's real colorability check). ``boundary_f_measure`` and friends
compare a prediction against an artistic *reference* drawing, which
``docs/DIAGNOSIS.md`` §4 shows is an incoherent way to gate acceptance (it
can score a reference translated by 3px *worse* than a mediocre engine,
and rewards drawing more ink outright). They remain useful here for
comparing candidate engines against each other and against the project's
reference images during development (see ``scripts/compare.py``), which is
why this module lives under ``scripts/`` instead of shipping in the
installed package.
"""
...
def boundary_f_measure(pred: np.ndarray, gt: np.ndarray, *, tolerance: int = 2) -> BoundaryMetrics:
    ...
    tolerance : int, optional
        ...
        degenerate-baseline tests in ``tests/scripts/test_metrics.py``,
        which is why the project default stays tight.
