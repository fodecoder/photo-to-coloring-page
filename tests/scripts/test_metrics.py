"""Tests for the dev-only reference-comparison metrics module (scripts/metrics.py).

See ``coloring_page.validate`` for the project's actual acceptance
criterion; this module's ``ink_coverage`` tests live in
``tests/test_validate.py`` alongside it now that ``ink_coverage`` moved
there.
"""
...
def test_boundary_f_measure_degenerate_baselines_lose_to_a_real_engine() -> None:
    ...
    gt = rasterize(CannyEngine().convert(photo), long_side_px=long_side)
    pred = rasterize(ChainedEngine().convert(photo), long_side_px=long_side)
...
def test_dilated_reference_is_not_ink_admissible() -> None:
    photo = make_synthetic_photo()
    gt = rasterize(CannyEngine().convert(photo), long_side_px=max(photo.shape[:2]))
